...
n_robots = strsplit(n_robots, ',');

pos_suscribers = [];

for i = 1:length(n_robots) 
    robot_suscriber = rossubscriber('/vicon/'+n_robots(i)+'/'+n_robots(i),"geometry_msgs/TransformStamped","struct");
    pos_suscribers = [pos_suscribers, robot_suscriber];
end 
...
while 1
    ...
    for i = 1:length(n_robots) 
        raw_data = receive(pos_suscribers(i),100);
        pos_data(n_robots(i)) = raw_data.Transform.Translation;
        disp(pos_data(n_robots(i)))
    end 
